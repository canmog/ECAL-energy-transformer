...
def main():
    ...
    meta = state.get("meta", cache_meta)
    require_meta(meta, ("n_concepts", "concept_names", "concept_mean",
                        "concept_std", "angle_mean", "angle_std",
                        "geometry_data_type"), cache_dir=checkpoint,
                 operation="angle probe checkpoint loading")
    require_meta(cache_meta, ("geometry_data_type",),
                 cache_dir=cfg.paths.cache_dir, operation="angle probe")
    if meta["geometry_data_type"] != cache_meta["geometry_data_type"]:
        raise ValueError("checkpoint and probe cache geometries differ")
    ...
    dataset = EcalTokens(
        ...
        task_mode="angle", required_fields={"angle", "concepts"},
        compute_fit_resid=False, operation="angle physics probe")
