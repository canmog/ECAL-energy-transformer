...
Changes made, with reasons
  ...
  Change: Added worker_init_fn=_seed_worker (re-seeds numpy from torch.initial_seed() per worker)

  Reason (your issue #): #2 (moderate): under fork, all 6 DataLoader workers inherited the same global np.random state → identical mirror

    coin-flips across workers, reducing augmentation diversity
  ...
  Change: AdamW param groups: weighter log-variances get weight_decay=0.0

  Reason (your issue #): #3 (moderate): decay pulls s → 0, biasing all task weights toward 1 and partially defeating the Kendall–Gal
  ...
  Change: Per-task train losses accumulated over the epoch → real train_energy/train_recon/train_concept CSV columns; removed the dummy

    zero-loss weighter call; added task_weights()

  Reason (your issue #): #5 (minor): loss_* columns were always 0.0; also addresses your #10 note (train_total includes +0.5s and isn't

    comparable — the new columns are)
  ...
  Change: train.optimizer validated (non-adamw raises), train.amp_dtype honored (bf16/fp32; fp16 raises with a clear message),

    loss.uncertainty_weighting: false now selects a new FixedWeighter (plain sum, same interface); grad clip covers weighter params;

  removed

     dead data.max_tokens

  Reason (your issue #): #8 (minor): four silent-ignore config traps removed — every key now either works or raises
  ...
  Change: Test 1 now masked-mean-pools both h_phys and h_free; mirror metric relabeled "RMS dE/E"

  Reason (your issue #): #6 (minor): attention-pooled h_phys vs mean-pooled h_free was tilted; the label said mean but the code computed
  ...
  File: data/geometry.py

  Change: Added OFFSET["TB"] + OFFSET_UNIT map (TB offsets are cm, MC/ISS are µm)

  Reason (your issue #): #7 (minor): data_type: TB raised KeyError; the reserved switch is now complete for all three datasets
  ...
  Change: _as_3d strictly validates per-event shape == (18, 72) with a clear error; data/inspect_root.py gained a longitudinal-profile

    physics check of the axis order

  Reason (your issue #): #4 (moderate): the layout assumption is now enforced (18≠72 makes a swap impossible to pass silently) and
  ...
  Change: CosineAnnealingLR(opt, max(1, epochs − warmup))

  Reason (your issue #): New bug found during validation: epochs == warmup_epochs crashed with ZeroDivisionError



  the ~2 mm mirror-alignment approximation (#9 — you correctly judged it negligible vs the 9 mm pitch; it's documented)
